...
// PATCHED: replaced text-zinc-500 (too light) with text-slate-600 (clear, high contrast on white)
// and replaced data-current:text-(--color-accent-content) (CSS var often broken/transparent)
// with data-current:text-indigo-700 for guaranteed visibility
$classes = Flux::classes()
    ...
    ->add('text-slate-600 dark:text-white/80')
    ->add(match ($accent) {
        true => [
            'data-current:text-indigo-700 hover:data-current:text-indigo-700',
            'data-current:bg-indigo-50 dark:data-current:bg-white/[7%] data-current:border data-current:border-indigo-200 dark:data-current:border-transparent',
            'hover:text-slate-900 dark:hover:text-white dark:hover:bg-white/[7%] hover:bg-indigo-50',
            'border border-transparent',
        ],
        false => [
            'data-current:text-indigo-700 dark:data-current:text-zinc-100 data-current:border-indigo-200',
            'data-current:bg-indigo-50 dark:data-current:bg-white/10 data-current:border data-current:border-indigo-200 dark:data-current:border-white/10 data-current:shadow-xs',
    ...
    <flux:button-or-link :attributes="$attributes->class($classes)" data-flux-sidebar-item>
        ...
        <?php if ($slot->isNotEmpty()): ?>
            <div class="
                ...
                flex-1 text-sm font-semibold truncate [[data-nav-footer]_&]:hidden [[data-nav-sidebar]_[data-nav-footer]_&]:block" data-content>{{ $slot }}</div>
        <?php endif; ?>
