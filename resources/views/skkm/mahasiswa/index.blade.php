<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('skkm.create') }}" class="group inline-flex items-center px-5 py-2.5 text-sm font-bold text-white bg-indigo-600 rounded-xl overflow-hidden transition-all hover:bg-indigo-700 shadow-lg hover:shadow-indigo-500/30 hover:scale-105">
    ...
    <div class="space-y-8">
        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-700 p-8 text-white shadow-2xl shadow-indigo-300/30">
                ...
                <div class="flex flex-col items-center">
                    <div class="relative w-36 h-36">
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                            <span class="text-3xl font-extrabold">{{ round($percentage) }}%</span>
                            <span class="text-indigo-200 text-[10px] font-semibold uppercase tracking-wider">Tercapai</span>
                        </div>
                    </div>
                    <p class="mt-3 text-sm font-semibold text-indigo-200">{{ $approvedPoints }} / {{ $targetKelulusan }} Poin</p>
                </div>

                {{-- Stats Grid --}}
                <div class="lg:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10 hover:bg-white/15 transition-colors">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-7 h-7 rounded-lg bg-emerald-400/20 flex items-center justify-center">
                            ...
                            <span class="text-[10px] font-semibold text-indigo-200 uppercase tracking-wider">Disetujui</span>
                        </div>
                        <p class="text-2xl font-extrabold">{{ $approvedCount }}</p>
                        <p class="text-[11px] text-indigo-300 mt-0.5">+{{ $approvedPoints }} poin</p>
                    </div>

                    <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10 hover:bg-white/15 transition-colors">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-7 h-7 rounded-lg bg-amber-400/20 flex items-center justify-center">
                                <svg class="w-4 h-4 text-amber-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                            </div>
                            <span class="text-[10px] font-semibold text-indigo-200 uppercase tracking-wider">Menunggu</span>
                        </div>
                        <p class="text-2xl font-extrabold">{{ $pendingCount }}</p>
                        <p class="text-[11px] text-indigo-300 mt-0.5">+{{ $pendingPoints }} poin</p>
                    </div>

                    <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10 hover:bg-white/15 transition-colors">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-7 h-7 rounded-lg bg-rose-400/20 flex items-center justify-center">
                            ...
                            <span class="text-[10px] font-semibold text-indigo-200 uppercase tracking-wider">Ditolak</span>
                        </div>
                        <p class="text-2xl font-extrabold">{{ $rejectedCount }}</p>
                        <p class="text-[11px] text-indigo-300 mt-0.5">perlu revisi</p>
                    </div>

                    <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10 hover:bg-white/15 transition-colors">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-7 h-7 rounded-lg bg-violet-400/20 flex items-center justify-center">
                                <svg class="w-4 h-4 text-violet-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg>
                            </div>
                            <span class="text-[10px] font-semibold text-indigo-200 uppercase tracking-wider">Sisa Target</span>
                        </div>
                        <p class="text-2xl font-extrabold">{{ $remainingPoints }}</p>
                        <p class="text-[11px] text-indigo-300 mt-0.5">poin lagi</p>
                    </div>
                </div>
            </div>
            ...
            <div class="relative z-10 mt-6 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <span class="text-xs font-medium text-indigo-300">Status Kelulusan:</span>
        ...
        <div class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 overflow-hidden">
            ...
            <div class="px-8 py-5 border-b border-slate-100 bg-slate-50/50">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <button data-filter="all" class="filter-btn active px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all bg-indigo-600 text-white shadow-sm">
                            Semua ({{ $submissions->count() }})
                        </button>
                        <button data-filter="disetujui" class="filter-btn px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all bg-slate-100 text-slate-600 hover:bg-emerald-50 hover:text-emerald-700">
                            Disetujui ({{ $approvedCount }})
                        </button>
                        <button data-filter="pending" class="filter-btn px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all bg-slate-100 text-slate-600 hover:bg-amber-50 hover:text-amber-700">
                            Menunggu ({{ $pendingCount }})
                        </button>
                        <button data-filter="ditolak" class="filter-btn px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all bg-slate-100 text-slate-600 hover:bg-rose-50 hover:text-rose-700">
            ...
            <div class="divide-y divide-slate-100">
                ...
                <div class="submission-row px-8 py-5 hover:bg-slate-50/60 transition-colors" data-status="{{ $sub->status_verifikasi }}">
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        <div class="flex items-start gap-4 flex-1 min-w-0">
                            ...
                            <div class="min-w-0 flex-1">
                                <h4 class="font-semibold text-slate-800 text-sm truncate">{{ $sub->nama_kegiatan }}</h4>
                                <div class="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
                                    ...
                                    <span class="inline-flex items-center text-xs text-slate-500">
                                        <svg class="w-3.5 h-3.5 mr-1 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A2 2 0 013 12V7a4 4 0 014-4z"></path></svg>
                                        {{ str_replace('_', ' ', ucwords($sub->pointRule->unsur ?? '-', '_')) }}
                                    </span>
                                    @if($sub->pointRule->tingkat)
                                    <span class="inline-flex items-center text-xs text-slate-500">
                                        <svg class="w-3.5 h-3.5 mr-1 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6h-8.5l-1-1H5a2 2 0 00-2 2zm9-13.5V9"></path></svg>
                                ...
                                @if($sub->catatan_dosen)
                                    <div class="mt-2 inline-flex items-start gap-1.5 px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-100">
                                        ...
                                        <span class="text-[11px] text-slate-600 leading-relaxed">{{ $sub->catatan_dosen }}</span>
                                    </div>
                                @endif
                            </div>
                        </div>
                ...
                <div class="px-8 py-16 text-center">
                    ...
                    <a href="{{ route('skkm.create') }}" class="inline-flex items-center mt-6 px-6 py-3 text-sm font-bold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 hover:shadow-indigo-300 hover:scale-105">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                        Ajukan Poin SKKM
                    </a>
                </div>
