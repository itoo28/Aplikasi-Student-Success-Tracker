<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            <h2 class="font-semibold text-2xl text-slate-800 leading-tight">CRUD Fakultas</h2>
            <a href="{{ route('admin.fakultas.create') }}" class="inline-flex items-center px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold shadow-sm">
                            ...
                            <tr class="hover:bg-slate-50/70 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.fakultas.edit', $item) }}" class="inline-flex px-3 py-2 rounded-lg bg-indigo-50 text-indigo-700 text-xs font-semibold hover:bg-indigo-100">Edit</a>
                                        <form action="{{ route('admin.fakultas.destroy', $item) }}" method="POST" onsubmit="return confirm('Hapus fakultas ini?')">
                                            @csrf
                                            @method('DELETE')
